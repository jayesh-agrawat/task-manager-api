...
router.get("/", (req, res) => {
  if (!taskArrData.length) return res.status(200).send("No Tasks Found");
...
router.post("/", (req, res) => {
  ...
  if (validator.validateTaskDetails(userProvidedDetails).status) {
    ...
    return res
      .status(201)
      .send(validator.validateTaskDetails(userProvidedDetails).message);
  }
  return res
    .status(400)
    .json(validator.validateTaskDetails(userProvidedDetails));
});
